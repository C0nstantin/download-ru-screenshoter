...
- **Скриншот области** — выделение произвольной прямоугольной области экрана с overlay-интерфейсом (macOS, Windows)
- **Скриншот всего экрана** — мгновенный захват всего экрана (macOS, Windows)
- **Скриншот окна** — захват конкретного окна (macOS: нативный пикер; Windows: Win32 API)
...
Режимы записи видео с экрана:

- **Запись экрана** — запись всего экрана (macOS: нативный пикер, аналог Cmd+Shift+5; Windows: Windows.Graphics.Capture API)
- **Запись области** (только macOS) — overlay для выбора прямоугольной области, затем запись выбранного участка
- **Запись окна** (только macOS) — выбор конкретного окна из списка, запись только этого окна

Во время записи:

- **Остановка записи через меню трея**
- Кнопка мьюта микрофона в UI записи (macOS)
...
> На Windows доступна только запись всего экрана. Запись области и окна в трее не показываются, т.к. эти режимы пока не реализованы. На Linux запись видео пока не реализована.
...
| Скриншот экрана | ✅ | ✅ | ⏳ |
| Скриншот окна | ✅ | ✅ | ⏳ |
| Мульти-монитор (overlay) | ✅ | ⏳ | ⏳ |
| Редактор с аннотациями | ✅ | ✅ | ✅ |
| Запись всего экрана | ✅ | ✅ | ⏳ |
| Запись области (видео) | ✅ | ❌ | ❌ |
| Запись окна (видео) | ✅ | ❌ | ❌ |
| Загрузка на download.ru | ✅ | ✅ | ✅ |
| Глобальные хоткеи | ✅ | ✅ | ✅ |

Легенда: ✅ работает · ⏳ частично / запланировано · ❌ недоступно
...
- [ ] **Запись области / окна на Windows** — реализовать через Windows.Graphics.Capture с пост-кропом
- [ ] **Запись видео на Linux** — реализовать через ffmpeg (`x11grab`)
